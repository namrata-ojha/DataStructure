...
/**
 * Created by namrataojha on 6/28/16. Questions are taken from this site
...
public class Solution {
   //1--- Given n of 1 or more, return the factorial of n, which is n * (n-1) * (n-2) ... 1. Compute the result recursively (without loops).
    ...
    // 2....We have a number of bunnies and each bunny has two big floppy ears. We want to compute the total number of ears across all the bunnies recursively
    ...
    // 4....We have bunnies standing in a line, numbered 1, 2, ... The odd bunnies (1, 3, ..) have the normal 2 ears.
    ...
    // 5.... We have triangle made of blocks. The topmost row has 1 block, the next row down has 2 blocks, the next row has 3 blocks, and so on.
    ...
    // 7.... Given a non-negative int n, return the count of the occurrences of 7 as a digit, so for example 717 yields 2. (no loops).
    // Note that mod (%) by 10 yields the rightmost digit (126 % 10 is 6), while divide (/) by 10 removes the rightmost digit (126 / 10 is 12).
    ...
    // 26...the Above Problem can also be done using string tools provided by java API
    ...
    //27... Given a string that contains a single pair of parenthesis, compute recursively a new string made of only of the parenthesis and their contents, so "xyz(abc)123" yields "(abc)".
    ...
    //30..... Given a string and a non-empty substring sub, compute recursively if at least n copies of sub appear in the string somewere, possibly with overlapping. N will be non-negative.
    ...
    public static void main (String[] args) {
        ...
        System.out.println( "Sum of digits : "+s.sumDigits(123));
        System.out.println("No. of x : "+ s.countX("x12xx"));
    }
